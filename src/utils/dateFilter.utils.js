 ...
 * Dates are parsed in Asia/Yangon timezone and converted to UTC for MongoDB query
...
export const createDateFilter = (
  ...
  if (startDate) {
    ...
    for (const format of dateFormats) {
      startMoment = moment.tz(startDate, format, TZ);
      if (startMoment.isValid()) {
        break;
      }
    }

    // If still not valid, try parsing without format (moment's default parsing)
    ...
    // For Date fields, set to start of day in Asia/Yangon
  ...
  if (endDate) {
    ...
    endMoment = null;
    for (const format of dateFormats) {
      endMoment = moment.tz(endDate, format, TZ);
    ...
    // For Date fields, set to end of day in Asia/Yangon
    // For string fields, we'll just extract the date part later
    if (!isStringField) {
      endMoment.endOf("day");
    }
